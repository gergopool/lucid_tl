...
def run(conf):
    ...
    save_filepath = os.path.join(save_folder, 'imagenet.h5')
    model_controller.model.save(save_filepath)

    print("\nFREEZING LAYERS\n")

    # Freeze and train first n epochs
    model_controller.freeze()
    model_controller.compile('adam')
    ...
    save_filepath = os.path.join(save_folder, 'finetuned.h5')
    model_controller.model.save(save_filepath)
